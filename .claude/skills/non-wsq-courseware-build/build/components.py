...
class Deck:
    ...
    def cover(self, logo=None):
        ...
        self.txt(s, Inches(0.9), Inches(4.65), Inches(12), Inches(1.4),
                 [[(f"Course Code: {C.COURSE_CODE}", 16, GREY, False)],
                  [(f"Conducted by {C.ORG}  ·  {C.UEN}", 14, GREY, False)]], space=6)
    ...
    def step_pair_slide(self, kicker, act_title, pair, total):
        """Two lab steps on one slide — same visual language as step_slide.

        `pair` is a list of (n, text) tuples, 1 or 2 items. Used for awareness-level
        courses where one-step-per-slide would inflate the deck.
        """
        s = self.head(self.slide(), act_title, kicker, TEAL)
        first, last = pair[0][0], pair[-1][0]
        label = f"STEP {first} OF {total}" if len(pair) == 1 else f"STEPS {first}-{last} OF {total}"
        self.txt(s, Inches(0.95), Inches(1.95), Inches(11), Inches(0.4),
                 [[(label, 13, GREY, True)]])
        top = 2.45
        for n, text in pair:
            self.oval(s, Inches(0.85), Inches(top), Inches(1.1), Inches(1.1), TEAL)
            self.txt(s, Inches(0.85), Inches(top), Inches(1.1), Inches(1.1),
                     [[(str(n), 28, WHITE, True)]], align=PP_ALIGN.CENTER, anchor=MSO_ANCHOR.MIDDLE)
            self.txt(s, Inches(2.25), Inches(top - 0.05), Inches(10.4), Inches(1.2),
                     [[(text, 17, INK, False)]], anchor=MSO_ANCHOR.MIDDLE)
            top += 1.55
        self.footer(s); return s
