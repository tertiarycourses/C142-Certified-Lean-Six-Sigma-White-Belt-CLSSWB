...
_d2=min(2,C.DAYS)
if C.DAYS==1:
    _tops=[_topic_line(_TB[n]) for n in _BY_DAY.get(1,[]) if n in _TB]
    _half=(len(_tops)+1)//2
    _left=[(f"Day 1 — {C.DAY_THEMES[1]}",0)]+_tops[:_half]
    _right=[("Day 1 (continued)",0)]+_tops[_half:]+[
...
two_col(f"Lesson Plan — {C.DAYS} Day{'s' if C.DAYS>1 else ''}, 8 hours/day",_left,_right,
 kicker="SCHEDULE",lhead="Day 1",rhead=_rhead)
